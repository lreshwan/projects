...
public:
    ...
    HASH_INDEX_T next() 
    {
        ...
        HASH_INDEX_T temp = ((dhstep_ * this->numProbes_) + this->start_) % this->m_;
        this->numProbes_++;
        return temp;
    }
};
...
{
    double total = sz + dataMember;

    if(total/table_.size() < alpha){}
    else
        resize();
...
{   
    ...
    for(int i = 0; i < (int)var.size(); i++){
        ...
        if(var[i] != NULL && var[i]->deleted){
            counter = 1;
        }
        else if(var[i] != NULL && !(var[i]->deleted)){
            counter = 2;
        }

        //deletes if deleted, or adds var to table
        switch (counter)
        {
        case 1:
            delete var[i];
        ...
        case 2:
            sz++;
            table_[this->probe(var[i]->item.first)]=var[i];
            break;
        }
    }
}
