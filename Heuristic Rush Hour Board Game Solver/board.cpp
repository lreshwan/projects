...
{
    if(myStack.empty()){
        throw std::underflow_error("No last move.");
    }
    else{
        vehicles_ = myStack.top();
        myStack.pop();
        updateBoard();
    }
...
bool Board::operator<(const Board& other) const
{
    string otherBoard = "";
    string myBoard = "";
    for(int i = 0; i < (int)this->grid_.size(); i++){
        for(int j = 0; j < (int)this->grid_[i].size(); j++){
            myBoard += this->grid_[i][j];
            otherBoard += other.grid_[i][j];
        }
    }
    if(otherBoard != myBoard){
        return true;
    }
    else{
        return false;
